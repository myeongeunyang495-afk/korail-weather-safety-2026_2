...
export async function handler() {
  ...
  for (const endpoint of ENDPOINTS) {
    try {
      ...
      const warnings = normalizeWarnings(items).filter(isTodayWarning);
      if (warnings.length) return resp(200, { warnings, source: endpoint, range: todayRange() });
  ...
  return resp(200, { warnings: [], attempts, range: todayRange() });
}
...
function todayRange() {
  const { start, end } = todayRangeDates();
  return { from: formatKst(start), to: formatKst(end) };
}
...
function todayRangeDates() {
  const now = new Date(Date.now() + 9 * 60 * 60 * 1000);
  const startUtc = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 0, 0, 0, 0) - 9 * 60 * 60 * 1000;
